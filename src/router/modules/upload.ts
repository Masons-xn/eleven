 ...
 * @LastEditTime: 2021-04-27 13:45:13
 * @LastEditors:
 */
  ...
  aa(request: { req: any; param?: object }): any {
    ...
    return new Promise((resolve: any, reject: any) => {
      form.parse(request.req, function (_err: Error, _fields: any, file: { [x: string]: { path: string }; tmpFile: { path: string } }) {
        let filePath = ""
        const openedFiles = form.openedFiles[0].name
        ...
        fs.rename(filePath, targetFile, (err) => {
          if (err) {
            ...
            resolve(
              innerService["attachments"].add({ path: fileUrl, orginName: openedFiles }).then((res: any) => {
                return {
                  code: "200",
                  res: Object.assign(res.res, { filename: openedFiles }),
                }
              })
            )
          }
