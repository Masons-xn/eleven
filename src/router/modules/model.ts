...
class Model extends Service {
  ...
  save(param: any): any {
    ...
    if (data) {
      ...
      isPowered = data.isPowered === false ? 0 : 1
    }
    if (data.stName && data.stTableName) {
      ...
      if (id) {
        ...
        modelSql = {
          sql: (_results?: any) => {
            return `update pt_model set stName = '${data.stName}', stTableName = '${data.stTableName}',isPowered = ${isPowered} where id = '${id}'`
          },
        }
      } else {
        id = guid()
        modelSql = {
          sql: (_results?: any) => {
            return `insert into pt_model (id,stName,stTableName,isPowered) values ('${id}', '${data.stName}','${data.stTableName}','${isPowered}')`
          },
        }
      }
      sql.push(modelSql)
      if (bases.length > 0) {
        ...
        bases.map((item: { id: any; createTime: any; updateTime: any; stName: any; stType: any; stColumnName: any; stlength: any }) => {
          insertData.push(
            `('${item.id ? item.id : guid()}','${this.dateFormat(item.createTime)}','${this.dateFormat()}','${item.stName}','${item.stType}','${item.stColumnName}','${item.stlength}','${id}')`
          )
        })
        sqlbase +=
          insertData.join(",") +
          ` on duplicate key update id=values(id),createTime=values(createTime),updateTime=values(updateTime),stName=values(stName),stColumnName=values(stColumnName),stlength=values(stlength),stModelId=values(stModelId)`
        baseSql = {
          sql: (_results: any) => {
            return sqlbase
          },
      ...
      if (rels.length > 0) {
        ...
        refsql +=
          insertData.join(",") +
          `on duplicate key update id=values(id),createTime=values(createTime),updateTime=values(updateTime),stSouModelID=values(stSouModelID),stDestModelID=values(stDestModelID),stName=values(stName),inRelation=values(inRelation)`
        relsSql = {
          sql: (_results: any) => {
            return refsql
          },
