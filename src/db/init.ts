...
function checkBase() {
  Promise.resolve(Mysql.exec(`SELECT A.stTableName,B.stName,B.stType,B.stColumnName,B.stlength
   FROM pt_model as A LEFT JOIN pt_model_basic_prop as B on A.id = B.stModelId`)).then((res) => {
    ...
    if (dbconfig.length) {
      ...
      for (let model in models) {
        Promise.resolve(Mysql.exec(`show tables LIKE '${model}'`)).then(_res => {
          if (_res.results.length === 0) {
            Promise.resolve(Mysql.exec(`CREATE TABLE ${model} (id VARCHAR(32) NOT NULL,PRIMARY KEY ( id ), createTime datetime, updateTime datetime,INDEX( id ))`)).then(_res => { }).then((_create) => {
...
let updateModel = (columns: Array<Column>, model: string, isBase: boolean = true): void => {
  Promise.resolve(Mysql.exec(`show columns from ${model}`)).then((_res) => {
    ...
    if (_res.results && _res.results.length > 0) {
      columnsInner = isBase ? _res.results.filter((item: any) => item.Field && item.Field !== 'id' && item.Field !== 'createTime' && item.Field !== 'updateTime' && (item.Field).indexOf("Id") === -1) : _res.results.filter((item: any) => item.Field && (item.Field).indexOf("Id") > -1)
    }
    let columnExit: Array<string> = []
    let columnBase: Array<string> = []
...
function checkRelation() {
  Promise.resolve(Mysql.exec(`SELECT
